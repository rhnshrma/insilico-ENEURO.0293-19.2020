...
class N_CA1: public Neuron{
 public:
  void ode_set(state_type &variables, state_type &dxdt, const double t,const unsigned index) {
    ...
    //Soma Currents
    double I_NaS = engine::neuron_value(index, "I_NaS");
    double I_KdS = engine::neuron_value(index, "I_KdS");
    double I_KcS = engine::neuron_value(index, "I_KcS");
    double I_KahpS = engine::neuron_value(index, "I_KahpS");
    double I_CaS = engine::neuron_value(index, "I_CaS");
    double I_Ext_S = engine::neuron_value(index, "I_Ext_S")/p;
    double I_coupling_S = -1*gc*(v_soma - v_dend)/p
    double I_Leak_S = engine::neuron_value(index, "I_Leak_S");

    //Dendrite Currents
    double I_KcD = engine::neuron_value(index, "I_KcD");
    double I_KahpD = engine::neuron_value(index, "I_KahpD");
    double I_CaD = engine::neuron_value(index, "I_CaD");
    double I_Ext_D = engine::neuron_value(index, "I_Ext_D")/(1-p);
    double I_coupling_D = -1*gc*(v_dend - v_soma)/(1-p)
    double I_Leak_D = engine::neuron_value(index, "I_Leak_D");
