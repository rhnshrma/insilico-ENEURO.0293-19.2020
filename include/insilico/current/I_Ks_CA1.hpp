 ...
 public:
   static void current(state_type &variables, state_type &dxdt, const double t, unsigned index) {
    double gna = 30.0, ena = 120.0;
                                                            // Declaring Constants
    unsigned v_soma_index = engine::neuron_index(index, "v_soma_CA1");
    unsigned h_soma_index = engine::neuron_index(index, "h_NaS_CA1");
    unsigned m_soma_index = engine::neuron_index(index, "m_NaS_CA1");
    ...
    dxdt[h_soma_index] = (h_inf - h)/tau_h;
     ...
     // Current
    engine::neuron_value(index, "I_NaS", (-1*(gna*pow(m,2)*h*(v-ena))));
