...
#ifndef INCLUDED_I_KahpD_CA1_HPP
#define INCLUDED_I_KahpD_CA1_HPP
...
class I_KahpD_CA1 {
 ...
 public:
  static void current(state_type &variables, state_type &dxdt, const double t, unsigned index) {
    ...
    unsigned q_dend_index = engine::neuron_index(index, "q_KahpD");
    unsigned Ca_dend_index = engine::neuron_index(index, "CaD");

    double v = variables[v_dend_index];
    double q = variables[q_dend_index];
    ...
    if (q>1) variables[q_dend_index]=1;
    else if(q<0) q=0;
    
    
                                                            // ODE set
    dxdt[q_dend_index] = (q_inf - q)/tau_q;


                                                            // Current
    engine::neuron_value(index,"I_KdD",(-1*(gk*q* (v - ek))));
